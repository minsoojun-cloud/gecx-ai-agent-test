...
# Setup Cloud Logging
logger = logging.getLogger("vertex-commerce-mcp")
logger.setLevel(logging.INFO)

try:
    logging_client = cloud_logging.Client()
    logging_client.setup_logging()
    cloud_logger = logging_client.logger("vertex-commerce-mcp")
except Exception:
    cloud_logger = None
...
@mcp.tool()
async def ai_agent_search_products(query: str, page_size: int = 5, filter: str = None, visitor_id: str = "mcp-agent-session") -> str:
    """
    ...
    # Cloud Logging 파라미터 디버그 출력
    debug_payload = {
        "event": "ai_agent_search_products_request",
        "placement": serving_config,
        "query": query,
        "page_size": page_size,
        "filter": filter,
    ...
    logger.info(f"[Debug] SearchRequest parameters: {json.dumps(debug_payload, ensure_ascii=False)}")
    if cloud_logger:
        try:
            cloud_logger.log_struct(debug_payload, severity="INFO")
        except Exception:
            pass
...
if __name__ == "__main__":
    ...
    # 상위 디렉토리(GECX)의 minsoo.html 등 정적 파일 서빙 추가
    parent_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    app.mount("/", StaticFiles(directory=parent_dir, html=True), name="static")
    
    uvicorn.run(app, host="0.0.0.0", port=int(os.environ.get("PORT", 8080)), proxy_headers=True, forwarded_allow_ips="*")