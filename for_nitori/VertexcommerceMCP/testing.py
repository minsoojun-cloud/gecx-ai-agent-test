...
REGION = "us-central1"
CLOUD_RUN_URL = "your mcp (cloud run) url"
...
1. Break the user's question into sub-tasks (e.g., find dress → check if it has pockets).
2. Search once or twice, then use 'get_product_details' on the most relevant IDs.
3. Do NOT keep searching with minor query variations. After 2 searches, work with what you have.
4. If the catalog doesn't have an exact match, recommend the closest alternatives.
5. Always end with a clear, helpful final answer summarizing your findings."""),
        ("human", "{input}"),
    ...
    agent_executor = AgentExecutor(
        ...
        early_stopping_method="generate",   # Forces a final answer even if max iterations hit
